...
# Calculate the base directory for the import. If 'model' is directly under 'TalkNet-asd',
# you would append 'TalkNet-asd' to the path.
base_dir = os.path.join(script_dir, '..')  # Adjust based on actual structure
base_dir = os.path.abspath(base_dir)  # Normalize the path
...
def visualization(tracks, scores, args):
	...
	savePath = os.path.join(args.pyworkPath, 'facesWithScore.pckl')
	with open(savePath, 'wb') as fil:
		pickle.dump(faces, fil)
	sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Faces with Scores extracted and saved in %s \r\n" %args.pyworkPath)
...
# 	sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Output video file saved in %s \r\n" %args.pyworkPath)
# 	output = subprocess.call(command, shell=True, stdout=None)
...
def main():
	...
	sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Started processing for video ASD Detection  \r\n")
